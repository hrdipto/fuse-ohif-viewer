...
const commandsModule = ({ servicesManager }) => {
	const customViewports = window.store.getState()['rootReducer']['viewports'].activeViewportIndex;

	const actions = {
		rotateViewport: ({ viewports, rotation }) => {
			const enabledElement = getEnabledElement(customViewports);
		...
		clearAnnotations: ({ viewports }) => {
			const element = getEnabledElement(customViewports);
			if (!element) {
				return;
			}
		...
		showDownloadViewportModal: ({ title, viewports }) => {
			const activeViewportIndex = customViewports;
		...
		jumpToImage: ({ StudyInstanceUID, SOPInstanceUID, frameIndex, activeViewportIndex }) => {
